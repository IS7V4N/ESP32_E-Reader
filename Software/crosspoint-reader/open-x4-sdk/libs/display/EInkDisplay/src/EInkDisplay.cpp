...
void EInkDisplay::refreshDisplay(const RefreshMode mode, const bool turnOffScreen) {
  ...
  if (mode == FULL_REFRESH) {
    ...
    displayMode |= 0xFC;  // Fast OTP waveform with TEMP_LOAD (reads internal sensor) + MODE_SELECT, correct polarity, no pre-inversion needed
  } else {  // FAST_REFRESH
  ...
  // Minimum wait guard: if BUSY pin is unreliable (e.g., floating on breadboard prototype),
  // waitWhileBusy() may return immediately while the waveform is still running.
  // Interrupting a half/full refresh mid-waveform leaves pixels in an undefined state,
  // causing inverted or ghost images that fade only with subsequent fast refreshes.
  // These minimums are conservative lower bounds measured on GDEQ0426T82 hardware.
  unsigned long minWaitMs = 0;
  ...
  if (elapsed < minWaitMs) {
    if (Serial) Serial.printf("[%lu]   BUSY returned in %lums (< %lums min) — BUSY pin may be unreliable, waiting...\n", millis(), elapsed, minWaitMs);
    delay(minWaitMs - elapsed);
  }
}
...
void EInkDisplay::deepSleep() {
  if (Serial) Serial.printf("[%lu]   Entering display deep sleep...\n", millis());

  // CMD_DEEP_SLEEP mode 1 (0x01): DCDC inactive, clock inactive, SRAM retained.
  // The controller shuts down analog/clock internally — no intermediate CMD_MASTER_ACTIVATION
  // needed. Running 0x03 + CMD_MASTER_ACTIVATION before this triggers an undocumented
  // SSD1677 "ANALOG_OFF_PHASE" waveform that visually degrades the sleep screen
  // (blur → white → gray dots) before the ESP32 powers down.
