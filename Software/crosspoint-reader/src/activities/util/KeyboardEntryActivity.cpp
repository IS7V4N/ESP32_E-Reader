...
void KeyboardEntryActivity::loop() {
  // Row navigation: long press Left = row up, long press Right = row down
  if (mappedInput.isPressed(MappedInputManager::Button::Left) &&
      mappedInput.getHeldTime() >= LONG_PRESS_ROW_MS && !longPressLeft) {
    longPressLeft = true;
  ...
  if (mappedInput.isPressed(MappedInputManager::Button::Right) &&
      mappedInput.getHeldTime() >= LONG_PRESS_ROW_MS && !longPressRight) {
    longPressRight = true;
  ...
  // Column navigation: short press Left/Right (fires on release, skipped if long press)
  if (mappedInput.wasReleased(MappedInputManager::Button::Left)) {
    if (!longPressLeft) {
      ...
      if (selectedRow == SPECIAL_ROW) {
        ...
        } else if (selectedCol >= BACKSPACE_COL) {
...
void KeyboardEntryActivity::clampSelectedCol() {
  const int maxCol = getRowLength(selectedRow) - 1;
